...
public class EmailController {
    ...
    public ResponseEntity<ApiResponseDto<String>> sendEmail(@RequestBody EmailRequest emailRequest) {
    ...
    public ResponseEntity<ApiResponseDto<String>> changePasswordEmail(@RequestBody EmailRequest emailRequest) {
    ...
    public ResponseEntity<ApiResponseDto<String>> verifyEmail(@RequestBody EmailRequest emailRequest) {
        if (emailVerificationService.verifyCode(emailRequest.getEmail(), emailRequest.getCode())) { // 인증 성공
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponseDto.error(ErrorStatus.VERIFY_EMAIL_FAILED)); // 400 Bad Req 상태로 실패 응답 반환
        }
    }
}